...
//ToonCoreStart
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_5271 = Set_UV0;
                float4 _ClippingMask_var = tex2D(_ClippingMask,TRANSFORM_TEX(node_5271, _ClippingMask));
...
////// Lighting:
                ...
                float Set_FinalShadowSample = saturate((node_8323 + ( (lerp( node_4315, (node_4315*saturate(((attenuation*node_4805)+node_4805+_Tweak_SystemShadowsLevel))), _Set_SystemShadowsToBase ) - node_583) * ((1.0 - _Set_1st_ShadePosition_var.rgb).r - node_8323) ) / (_BaseColor_Step - node_583)));
                ...
                float4 _Set_HighColorPosition_var = tex2D(_Set_HighColorPosition,TRANSFORM_TEX(node_4506, _Set_HighColorPosition));
                float node_1331 = 0.5*dot(halfDirection,lerp( i.normalDir, normalDirection, _Is_NormalMapToHighColor ))+0.5; //  Specular
                float node_6262 = ((1.0 - _Set_HighColorPosition_var.rgb).r+lerp( (1.0 - step(node_1331,(1.0 - _HighColor_Power))), pow(node_1331,exp2(lerp(11,1,_HighColor_Power))), _Is_SpecularToHighColor ));
                float3 node_5205 = (lerp( _HighColor.rgb, (_HighColor.rgb*Set_LightColor), _Is_LightColor_HighColor )*node_6262);
                float node_2595 = Set_FinalShadowSample;
                float3 Set_HighColor = (lerp( saturate((node_1702-node_6262)), node_1702, _Is_BlendAddToHiColor )+lerp( node_5205, (node_5205*((1.0 - node_2595)+(node_2595*_TweakHighColorOnShadow))), _Is_UseTweakHighColorOnShadow ));
                float3 node_3558 = Set_HighColor;
                float node_7879 = pow((1.0 - dot(lerp( i.normalDir, normalDirection, _Is_NormalMapToRimLight ),viewDirection)),exp2(lerp(3,0,_RimLight_Power)));
                float node_8606 = 1.0;
                float node_9273 = 0.0;
                float3 Set_RimLight = (lerp( _RimLightColor.rgb, (_RimLightColor.rgb*Set_LightColor), _Is_LightColor_RimLight )*saturate(lerp( (node_9273 + ( (node_7879 - _RimLight_InsideMask) * (node_8606 - node_9273) ) / (node_8606 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_7879), _RimLight_FeatherOff )));
                float3 _RimLight_var = lerp( node_3558, (node_3558+Set_RimLight), _RimLight );
                ...
                float3 finalColor = (saturate(lerp( _RimLight_var, lerp( (_RimLight_var*node_4172), (_RimLight_var+node_4172), _Is_BlendAddToMatCap ), _MatCap ))+(DecodeLightProbe( normalDirection )*_GI_Intensity));
