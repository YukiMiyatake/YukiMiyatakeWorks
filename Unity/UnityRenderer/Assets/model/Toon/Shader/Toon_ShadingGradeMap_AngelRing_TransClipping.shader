...
//ToonCoreStart
        Pass {
            ...
            uniform fixed _Is_LightColor_2ndShade;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 node_2394 = Set_UV0;
                float4 _ClippingMask_var = tex2D(_ClippingMask,TRANSFORM_TEX(node_2394, _ClippingMask));
...
////// Lighting:
                ...
                float3 node_3002 = Set_LightColor;
                float3 Set_BaseColor = lerp( node_482, (node_482*node_3002), _Is_LightColor_Base );
                ...
                float3 _Is_LightColor_1st_Shade_var = lerp( node_7531, (node_7531*node_3002), _Is_LightColor_1st_Shade );
                ...
                float3 Set_FinalBaseColor = lerp( lerp(node_4770,lerp(_Is_LightColor_1st_Shade_var,lerp( node_5308, (node_5308*node_3002), _Is_LightColor_2ndShade ),Set_ShadeShadowMask),Set_FinalShadowMask), node_4770, _Is_1st_ShadeColorOnly );
                ...
                float node_9686 = 1.0;
                float node_6470 = 0.0;
                float3 Set_RimLight = (lerp( _RimLightColor.rgb, (_RimLightColor.rgb*Set_LightColor), _Is_LightColor_RimLight )*saturate(lerp( (node_6470 + ( (node_3992 - _RimLight_InsideMask) * (node_9686 - node_6470) ) / (node_9686 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_3992), _RimLight_FeatherOff )));
                ...
                float3 node_3312 = lerp(mul( UNITY_MATRIX_V, float4(i.normalDir,0) ).xyz.rgb,float3(0,0,1),_AR_OffsetU);
                float2 node_6954 = float2((node_3312.r*0.5+0.5),lerp(i.uv1.g,(node_3312.g*0.5+0.5),_AR_OffsetV));
                float4 _AngelRing_Sampler_var = tex2D(_AngelRing_Sampler,TRANSFORM_TEX(node_6954, _AngelRing_Sampler));
                float3 node_6611 = (_AngelRing_Sampler_var.rgb*_AngelRing_Color.rgb);
                float3 Set_AngelRing = lerp( node_6611, (node_6611*Set_LightColor), _Is_LightColor_AR );
